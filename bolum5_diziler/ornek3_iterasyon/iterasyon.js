...
const ara = prompt(
  'aramak istenilen hayvan türünü giriniz:'
).toLowerCase();
...
sayi == 0
  ? console.log(
      'Aradiginiz hayvan bulunamadi'
    )
  : console.log(
      `Aradiginiz ${ara} 'den ${sayi} adet bulunmaktadir`
    );
// ======================================================
//                   FOR OF DÖNGÜSÜ
// ======================================================
...
// gösteriniz.
tlFiyatlar = [
  120, 340, 550, 245, 322.5, 789,
];
// Kur inputlarini al.
const dolarKur =
  document.querySelector('.dolar');
const yuroKur =
  document.querySelector('.yuro');
//dolar ve euro fiyatlari yazacak elemanlari al
const dolar = document.querySelector(
  '.dolar-fiyat'
);
const yuro = document.querySelector(
  '.yuro-fiyat'
);
...
dolarKur.onchange = function () {
  dolarKur.value < 0
    ...
    : (dolar.innerHTML = tlFiyatlar.map(
        (tl) =>
          Math.round(
            tl / dolarKur.value
          )
...
yuroKur.onchange = function () {
  yuroKur.value < 0
    ? alert("Kur 0'dan küçük olamaz")
    : (yuro.innerHTML = tlFiyatlar.map(
        (tl) =>
          Math.round(tl / yuroKur.value)
      ));
};
//-------------- ÖRNEK -------------------
  ...
  (deger, indis) => {
    if (deger < 250) {
      return `${
        ...
        deger * 1.1
      } <br> `; //! <br> koymamızın sebebi bir alt satıra yazdırabilmemiz için.
    } else {
      return `${
        indis + 1
      }.ürünün zamli fiyati : ${
...
const stringZamliTLFiyatlar =
  zamliTlFiyatlar.join(''); //* join yapmazsak yazımız hem virgüllü oluyor. Virgülden kurtasdık.
console.log(stringZamliTLFiyatlar);
document.querySelector(
  '.zamli-fiyat'
).innerHTML = stringZamliTLFiyatlar;
// ======================================================
//                       FILTER METODU
// ======================================================
// tlFiyatlar listesinde fiyatı 250 TL den az olanlari ayri
// bir diziye saklayalim.
const kucuk250 = tlFiyatlar.filter(
  (d) => d < 250
);
...
const toplamMaas = maaslar.reduce(
  (x, y) => x + y,
  0
);
console.log(toplamMaas);
const toplamMaas1 = maaslar.reduce(
  (x, y, i) => {
    console.log(
      `${i} iterasyon : ${x}`
    );
