...
document
  .querySelector('.kontrol')
  .addEventListener('click', () => {
    const tahmin = Number(
      document.querySelector('.tahmin')
        .value
    );
    let mesaj =
      document.querySelector('.mesaj');

    // Tahmin girilmediyse bilgi ver
    if (!tahmin) {
      mesaj.textContent =
        'Lütfen Bir Tahmin Giriniz';
      // Tahmin doğru ise
    } else if (
      tahmin == rasgeleSayı &&
      skor > 1
...
document
  .querySelector('.tekrar')
  .addEventListener('click', () => {
    skor = 10;
    rasgeleSayı = Math.floor(
      Math.random() * 20 + 1
    );
    document.querySelector(
      '.mesaj'
