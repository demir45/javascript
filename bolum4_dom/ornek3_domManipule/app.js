...
const sonDiv = document.querySelector(
  '.input-div'
);
const h1 = document.createElement('h1');
const yazi = document.createTextNode(
  'Programla Dilleri'
);
h1.appendChild(yazi);
sonDiv.after(h1); //input-div'in sonrasina h1'i ekliyoruz.
h1.className = 'yeni-h1'; // oluşturduğumuz h1 elemanına calssName atadık.
...
document.querySelector('.sil').onclick =
  function () {
    const liste =
      document.querySelector('.liste');
    // liste.removeChild(
    //   liste.firstElementChild //?  ilk elemanı sil
    // );
    liste.removeChild(
      liste.lastElementChild //? son elemanı sil
    );
  };
